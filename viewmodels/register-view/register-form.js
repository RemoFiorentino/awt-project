...
exports.createInitializer = function (options) {
    function ViewModel(context, req) {
        ...
        this.fields = {};
        this.status = 'ready';
        this.fields['account-type'] = req.query[this.id + '$f$account-type'] || '';
        this.fields['fullname'] = req.query[this.id + '$f$fullname'] || '';
        this.fields['password'] = req.query[this.id + '$f$password'] || '';
        this.fields['username'] = req.query[this.id + '$f$username'] || '';
    };
    ...
    ViewModel.prototype.compute = function () {
        ...
        if (this.status === 'clear') {
            return this.computing = Promise.all([
              this.context.elements['on-register-submit'].compute()
            ])
            .then(compute);
        } else {
        return this.computing = Promise.all([
